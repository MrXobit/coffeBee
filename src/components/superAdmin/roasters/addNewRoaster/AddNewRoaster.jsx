...
const AddNewRoaster = () => {
  ...
  const [socials, setSocials] = useState(() => {
     const saved = localStorage.getItem('addRoasterSocials');
  ...
  const [formData, setFormData] = useState(() => {
    const saved = localStorage.getItem('addRoasterFormData');
    return saved ? JSON.parse(saved) : {
      name: '',
      country: '',
      description: '',
  ...
  useEffect(() => {
    localStorage.setItem('addRoasterFormData', JSON.stringify(formData));
  }, [formData]);

  useEffect(() => {
    localStorage.setItem('addRoasterSocials', JSON.stringify(socials));
  }, [socials]);
  ...
  const handleEddRoaster = async(e) => {
    e.preventDefault();
    setLoading(true);
    try {
      ...
      const idRoaster = uuidv4();
      const imageRef = ref(storage, `roasters/${idRoaster}/logo`);
      ...
      localStorage.removeItem('addRoasterFormData');
      localStorage.removeItem('addRoasterSocials');      
      await setDoc(doc(db, "roasters", newRoaster.id), newRoaster);
      setError(null);
      setFormData({
        name: '',
        country: '',
  ...
  return (
    <div className="add-new-roaster-container">
      <h1 className="add-new-roaster-title">Add New Roaster</h1>
      <form onSubmit={handleEddRoaster} className="add-new-roaster-form">
    ...
    <p className="add-new-roaster-beans-upload-prompt">
      Drag and drop a file or select it via the button
        ...
        <button disabled={loading} className='add-new-roaster-main-btn'>
          {loading ? 'loading...' : 'Add New Roaster'}
        </button>
      </form>
    </div>
  );
